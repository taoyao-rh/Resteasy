...
public class JsonBindingTest {
   ...
   public static Archive<?> deploy() {
      ...
      //WebArchive war = ShrinkWrap.create(WebArchive.class, JsonBindingTest.class.getSimpleName());
      war.addAsManifestResource(new StringAsset("Dependencies: org.jboss.resteasy.resteasy-json-binding-provider services"), "MANIFEST.MF");
      war.addClass(JsonBindingTest.class);
      Archive<?> res = TestUtil.finishContainerPrepare(war, null, JsonBindingResource.class, Cat.class);
      //((ManifestContainer<WebArchive>)res).setManifest(new StringAsset("Manifest-Version: 1.0\n Dependencies: org.jboss.resteasy.resteasy-json-binding-provider services \n"));
      war.as(ZipExporter.class).exportTo(new File("/home/jimma/code/resteasy-master/Resteasy/testsuite/integration-tests/target", war.getName()), true);
      return res;
   ...
   public void doTestCat() throws Exception {
      ...
      String response = target.request().post(entity).readEntity(String.class);
      Assert.assertEquals("JsonBindingProvider is not enabled", response, "{\"color\":\"ginger\",\"domesticated\":true,\"name\":\"Alfred\",\"sort\":\"semi-british\"}");
